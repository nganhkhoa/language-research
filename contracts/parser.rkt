  ...
  (cond
    ...
    [(list? s-expr)
     (match s-expr
       ...
       [(list fst snd rd)
        (cond
          [(and (is-lambda? fst) (symbol? snd)) (function snd (parse rd))]
          [(and (is-fix? fst) (symbol? snd)) (fixpoint snd (parse rd))]
...
(define (parse-decl s-expr)
  ...
  (match s-expr
    ;; no contract
    [(list 'val 'rec x '= e) (val x (f-contract (function '_ (bool true))) (parse e))]
    ;; with contract
    [(list 'val 'rec x ': c '= e) (val x (parse c) (parse e))]
...
(module+ test
  ...
  (test (parse `"hello world") (str "hello world"))

  (test (parse `{1 + 1}) (rop '+ (num 1) (num 1)))

  (test (parse `{1 >= 2}) (aop '>= (num 1) (num 2)))

  (test (parse `{1 :: []}) (concat (num 1) (nil)))

  (test (parse `{g (lambda x 25)}) (application (id 'g) (function 'x (num 25))))

  (test (parse `{(lambda x (x + 1)) 1}) (application (function 'x (rop '+ (id 'x) (num 1))) (num 1)))

  (test (parse `{g (fix x 25)}) (application (id 'g) (fixpoint 'x (num 25))))

  (test (parse `{(fix x (x + 1)) 1}) (application (fixpoint 'x (rop '+ (id 'x) (num 1))) (num 1)))

  (test (parse `{hd (1 :: [])}) (head (concat (num 1) (nil))))

  (test (parse `(if #t then 1 else 2)) (condition (bool #t) (num 1) (num 2)))
  ...
  (test (parse-decl `{val
        ...
        (val 'bet0_99
             (nil)
             (function 'x
                       (condition (aop '>= (num 99) (id 'x)) (aop '>= (id 'x) (num 0)) (bool #f)))))

  (test (parse-decl `{val rec g : ((gt9 -> bet0_99) -> bet0_99) = (lambda f (f 0))})
        (val 'g
             (h-contract (h-contract (id 'gt9) (id 'bet0_99)) (id 'bet0_99))
