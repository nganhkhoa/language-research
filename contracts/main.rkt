...
(define (program-text->datum program-text)
  (let ([wrapped-text (format "(~a)" program-text)]) (read (open-input-string wrapped-text))))

(define (try-parse program-expr)
  (define (try-parse-element s-expr)
    (with-handlers ([exn:fail? (lambda (_) (parse s-expr))])
      (parse-decl s-expr)))
  (map try-parse-element program-expr))

(define (main-program args)
  (cond
    [(empty? args) (printf "Usage: racket main.rkt <program-file.rkt>~n")]

    [(equal? 1 (length args))
     (let*-values ([(file-path) (first args)]
    ...
    [else (printf "Too many arguments.~n")]))
