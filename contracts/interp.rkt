...
(define (interp (expr rz-expr?) (decls (listof rz-decl?)))
  (type-case rz-expr expr
             ...
             [condition (e t-branch f-branch)
                        (type-case rz-value (interp e decls)
                          [rz-bool (b) (cond
             ...
             [concat (left right)
                     (rz-list (interp left decls) (interp right decls))]

             [head (lst)
                   (type-case rz-value (interp lst decls)
                     [rz-list (hd _) hd]
                     [rz-nil ()
             ...
             [fixpoint (param body)
                       (interp (subst param (fixpoint param body) body) decls)]

             [application (funclike arg)
                          (define f (interp funclike decls))
                          (define a (interp arg decls))
                          (printf "apply ~a ~a\n" f a)
                          (type-case rz-value f
...
(define (obligation-reduce (obligation rz-value?) (decls (listof rz-decl?)))
  ...
  (type-case rz-value obligation
             [rz-obligation (v c in out)
                            ;; if e is rz-app then move on and apply the function
                            (define reduced-v (type-case rz-value v
                                          [rz-app (function argument)
                                                  (type-case rz-value function
                                                             [rz-function (param body)
                                                                          (interp (subst param argument body) decls)]
                            ...
                            (cond
                              [(rz-hcontract? c)
                               ...
                               (rz-obligation reduced-v c in out)]
                              [(rz-fcontract? c)
                               ;; flat contract, e must be a basic value
                               (type-case rz-value (rz-fcontract-c c)
                                          ;; check contract
                                          [rz-function (param body)
                                                       (define condition (interp (subst param reduced-v body) decls))
                                                       (type-case rz-value condition
                                                                  [rz-bool (b) (if b reduced-v (error 'interp "rize contract violation contract:~a value:~a at:~a" c reduced-v in))]
...
(define (as-expr (v rz-value?))
  (type-case rz-value v
             ...
             [rz-nil () (nil)]
             [rz-obligation (v c in out)
                            (obligation (as-expr v) (as-expr c) in out)]
...
(define (subst arg maybe-value expr)
  (define value (if (rz-expr? maybe-value) maybe-value (as-expr maybe-value)))
  (type-case rz-expr expr
